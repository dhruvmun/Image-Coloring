...
class CNN:
	...
	def initialize_parameters(self):
		...
		std = 0.5
		# Con-Layer1 Filter
		self.parameters['conv_layer1_weights'] = np.random.normal(mean, std, (
			self.filter_size, self.filter_size, self.input_channels, self.no_of_filters1))
		# Layer1 Bias
		self.parameters['conv_layer1_biases'] = np.zeros((1, self.no_of_filters1))
		# Layer2 Filter
		self.parameters['conv_layer2_weights'] = np.random.normal(mean, std, (
			self.filter_size, self.filter_size, self.no_of_filters1, self.no_of_filters2))
		# Layer2 Bias
		self.parameters['conv_layer2_biases'] = np.zeros((1, self.no_of_filters2))
		# Decon-Layer1 Filter
		self.parameters['deconv_layer1_weights'] = np.random.normal(mean, std, (
			self.filter_size, self.filter_size, self.no_of_filters2, self.no_of_filters1))
		# Decon Layer Bias
		self.parameters['deconv_layer1_biases'] = np.zeros((1, self.no_of_filters2))
		# Decon Layer2 Filter
		self.parameters['deconv_layer2_weights'] = np.random.normal(mean, std, (
			self.filter_size, self.filter_size, self.no_of_filters1, self.output_channels))
		# Decon Layer2 Bias
	...
	def forward_prop(self, image):
		...
		print(image.shape)
		conv_layer1 = self.conv_layer(image, self.parameters['conv_layer1_weights'],
									  self.parameters['conv_layer1_biases'], True)
		print(conv_layer1.shape)
		maxpool1 = self.maxpool(conv_layer1)
		print(maxpool1.shape)
		conv_layer2 = self.conv_layer(maxpool1, self.parameters['conv_layer2_weights'],
									  self.parameters['conv_layer2_biases'], True)
		print(conv_layer2.shape)
		maxpool2 = self.maxpool(conv_layer2)
		print(maxpool2.shape)
		unpool1 = self.unPooling(maxpool2)
		print(unpool1.shape)
		deconv_layer1 = self.deconv_layer(unpool1, self.parameters['deconv_layer1_weights'],
										  self.parameters['deconv_layer1_biases'])
		print(deconv_layer1.shape)
		unmaxpool2 = self.unPooling(deconv_layer1)
		print(unmaxpool2.shape)
		deconv_layer2 = self.deconv_layer(unmaxpool2, self.parameters['deconv_layer2_weights'],
										  self.parameters['deconv_layer2_biases'])
